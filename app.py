...
# ============================================
# PESTAÑA 3: HISTORIAL CLÍNICO - VERSIÓN FINAL
# ============================================
...
with tab_seg3:
    ...
    if not st.session_state.seguimiento_paciente:
        st.info("🔍 Seleccione un paciente en la pestaña 'Buscar Paciente' para ver su historial.")
    else:
        paciente = st.session_state.seguimiento_paciente
        ...
        st.markdown(f"""
        <div style="background: #f3e8ff; padding: 1.5rem; border-radius: 10px; border-left: 5px solid #9333ea; margin-bottom: 1rem;">
            <h3 style="margin: 0; color: #6b21a8;">📊 HISTORIAL DE: {paciente.get('nombre_apellido', '').upper()}</h3>
            <div style="display: flex; justify-content: space-between; margin-top: 10px;">
                <span><strong>DNI:</strong> {paciente.get('dni')}</span>
                <span><strong>Edad:</strong> {paciente.get('edad_meses')} meses</span>
                <span><strong>Región:</strong> {paciente.get('region')}</span>
            </div>
        </div>
        """, unsafe_allow_html=True)
        ...
        with col_acc1:
            if st.button("🔄 Actualizar Historial", type="primary", use_container_width=True):
                st.rerun()
        with col_acc2:
            st.button("📝 Nuevo Seguimiento", use_container_width=True, disabled=True) # Referencia visual
        with col_acc3:
            if st.button("🔍 Cambiar Paciente", use_container_width=True):
                st.session_state.seguimiento_paciente = None
                st.rerun()

        if not historial:
            st.warning("⚠️ No se encontraron controles registrados para este paciente.")
        else:
            # --- MÉTRICAS RESUMEN ---
            df_historial = pd.DataFrame(historial)
            col_m1, col_m2, col_m3, col_m4 = st.columns(4)
            col_m1.metric("Total controles", len(df_historial))
            col_m2.metric("Hb promedio", f"{df_historial['hemoglobina_actual'].mean():.1f} g/dL")
            col_m3.metric("Último control", df_historial['fecha_seguimiento'].iloc[0])
            col_m4.metric("Clasificación actual", df_historial['clasificacion_actual'].iloc[0])

            # --- GRÁFICA DE EVOLUCIÓN (Usando Plotly para que sea similar a tu imagen) ---
            st.subheader("📈 Evolución de Hemoglobina")
            import plotly.express as px
            
            fig = px.line(df_historial, x="fecha_seguimiento", y="hemoglobina_actual", 
                         markers=True, title="Tendencia de Hemoglobina (g/dL)")
            # Línea roja de referencia (límite anemia)
            fig.add_hline(y=11.0, line_dash="dash", line_color="red", annotation_text="Límite Anemia (11.0)")
            st.plotly_chart(fig, use_container_width=True)

            # --- TABLA DE CONTROLES REGISTRADOS ---
            st.subheader("📋 Controles Registrados")
            st.dataframe(df_historial[[
                'fecha_seguimiento', 'tipo_seguimiento', 'hemoglobina_actual', 
                'clasificacion_actual', 'usuario_responsable', 'proximo_control'
            ]], use_container_width=True)

            # --- SECCIÓN DE EXPORTACIÓN (Como en tu imagen) ---
            st.markdown("---")
            ...
            with col_exp1:
                csv = df_historial.to_csv(index=False).encode('utf-8')
                st.download_button("📊 Exportar a CSV", data=csv, file_name=f"historial_{paciente['dni']}.csv", mime="text/csv", use_container_width=True)
            
            with col_exp2:
                if st.button("📄 Generar Informe PDF", type="primary", use_container_width=True):
                    st.info("Generando PDF... Por favor espere.")
                    # Aquí llamarías a tu función de creación de PDF (ej: FPDF o ReportLab)
            
            with col_exp3:
                st.button("🖨️ Vista de Impresión", use_container_width=True)
